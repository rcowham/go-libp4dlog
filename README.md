...
It is a single executable `log2sql` which will parse a text p4d text log file and by default it will generate a Sqlite3 database and VictoriaMetrics metrics file (historical metrics).

It is considerably faster (but compatible with) the `log2sql.py` script mentioned below.
...
will produce a `log2020-02-01.db` (stripping off `.gz` and `.log` from name and appending `.db`) and `log2020-02-01.metrics`.
...
When it has started you will be able to connect to Grafana with [this link](http://localhost:3000).
